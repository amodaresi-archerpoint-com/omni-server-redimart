...
namespace LSOmni.DataAccess.BOConnection.CentrAL.Dal
{
    public class ContactRepository : BaseRepository
    {
        ...
        public List<ReplCustomer> ReplicateMembers(int batchSize, bool fullReplication, ref string lastKey, ref string maxKey, ref int recordsRemaining)
        {
            ...
            List<JscKey> keys = GetPrimaryKeys("Member Contact$5ecfc871-5d82-43f1-9c54-59685e82318d");

            // get records remaining
            string sql = string.Empty;
            sqlcolumns += ",ma.[Club Code],ma.[Scheme Code],ma.[Linked To Customer No_]";
            sqlfrom += " LEFT JOIN [" + navCompanyName + "Member Account$5ecfc871-5d82-43f1-9c54-59685e82318d] ma ON ma.[No_]=mt.[Account No_]";
        ...
        public List<MemberContact> ContactSearch(ContactSearchType searchType, string search, int maxNumberOfRowsReturned)
        {
            ...
            switch (searchType)
            {
                ...
                case ContactSearchType.CardId:
                    col += ",mc.[Card No_] ";
                    from += " LEFT JOIN [" + navCompanyName + "Membership Card$5ecfc871-5d82-43f1-9c54-59685e82318d] mc on mc.[Contact No_]=mt.[Contact No_] ";
                ...
                case ContactSearchType.UserName:
                    col += ",mlc.[Login ID]";
                    from += " LEFT JOIN [" + navCompanyName + "Membership Card$5ecfc871-5d82-43f1-9c54-59685e82318d] mc on mc.[Contact No_]=mt.[Contact No_]" +
                            " LEFT JOIN [" + navCompanyName + "Member Login Card$5ecfc871-5d82-43f1-9c54-59685e82318d] mlc on mlc.[Card No_]=mc.[Card No_]";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "SELECT mt.[ID],mt.[Security Token],mt.[Friendly Name],mt.[Status],mt.[Reason Blocked],mt.[Date Blocked],mt.[Blocked By]" +
                                         " FROM [" + navCompanyName + "Member Device$5ecfc871-5d82-43f1-9c54-59685e82318d] mt WHERE mt.[ID]=@id";
        ...
        public bool IsUserLinkedToDeviceId(string userName, string deviceId)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "SELECT [Login ID] FROM [" + navCompanyName + "Member Login Device$5ecfc871-5d82-43f1-9c54-59685e82318d] WHERE [Login ID]=@Lid " +
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT mt.[Card No_],mt.[Contact No_],mt.[Club Code],mt.[Status],mt.[Linked to Account]," +
                                 ...
                                 "LEFT JOIN [" + navCompanyName + "Member Login Card$5ecfc871-5d82-43f1-9c54-59685e82318d] ml on mt.[Card No_] = ml.[Card No_]" +
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "SELECT mt.[Card No_],mt.[Contact No_],mt.[Club Code],mt.[Status]," +
                                 ...
                                 "FROM [" + navCompanyName + "Membership Card$5ecfc871-5d82-43f1-9c54-59685e82318d] AS mt WHERE mt.[Card No_]=@id";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "SELECT mt.[No_],mt.[Scheme Code],mt.[Linked To Customer No_],mt.[Status],mt.[Account Type],mt.[Blocked]," +
                                          "(SELECT SUM([Remaining Points]) FROM [" + navCompanyName + "Member Point Entry$5ecfc871-5d82-43f1-9c54-59685e82318d] " +
                                          "WHERE [Account No_]=@id AND [Open]=1) AS Sum1," +
                                          "(SELECT SUM([Points in Transaction]) FROM [" + navCompanyName + "Member Process Order Entry$5ecfc871-5d82-43f1-9c54-59685e82318d] " +
                                          "WHERE [Account No_]=@id AND [Date Processed]='1753-01-01') AS Sum2 " +
                                          "FROM [" + navCompanyName + "Member Account$5ecfc871-5d82-43f1-9c54-59685e82318d] mt WHERE mt.[No_]=@id";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "SELECT mt.[Code],mt.[Description],mt.[Club Code],mt.[Update Sequence],mt.[Next Scheme Benefits]," +
                                          ...
                                          "LEFT JOIN [" + navCompanyName + "Member Scheme$5ecfc871-5d82-43f1-9c54-59685e82318d] up " +
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "SELECT mt.[Code],mt.[Description]" +
                                          "FROM [" + navCompanyName + "Member Club$5ecfc871-5d82-43f1-9c54-59685e82318d] mt WHERE mt.[Code]=@id";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "SELECT mt.[Code],a.[Description],a.[Attribute Type],a.[Default Value],a.[Mandatory],a.[Visible Type],a.[Lookup Type] " +
                                          "FROM [" + navCompanyName + "Member Attribute Setup$5ecfc871-5d82-43f1-9c54-59685e82318d] mt " +
                                          "JOIN [" + navCompanyName + "Member Management Setup$5ecfc871-5d82-43f1-9c54-59685e82318d] mms ON mms.[Mobile Default Club Code]=mt.[Club Code] " +
                                          "JOIN [" + navCompanyName + "Member Attribute$5ecfc871-5d82-43f1-9c54-59685e82318d] a ON a.[Code]=mt.[Code] AND a.[Visible Type]=0 AND a.[Lookup Type]=0";
        ...
        public virtual List<Profile> ProfileGetByCardId(string id, bool includeAll)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "SELECT mt.[Code],mt.[Description],mt.[Attribute Type],mt.[Default Value],mt.[Visible Type],mt.[Lookup Type],mt.[Mandatory],v.[Attribute Value] " +
                                          "FROM [" + navCompanyName + "Member Attribute$5ecfc871-5d82-43f1-9c54-59685e82318d] mt " +
                                          "JOIN [" + navCompanyName + "Membership Card$5ecfc871-5d82-43f1-9c54-59685e82318d] mc ON mc.[Card No_]=@id " +
                                          "JOIN [" + navCompanyName + "Member Attribute Value$5ecfc871-5d82-43f1-9c54-59685e82318d] v ON v.[Attribute Code]=mt.[Code] AND v.[Account No_]=mc.[Account No_]";
                    if (includeAll == false)
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT mt.[Code],mt.[Description],mt.[Attribute Type],mt.[Default Value],mt.[Mandatory],mt.[Visible Type],mt.[Lookup Type] " +
                                          ...
                                          "JOIN [" + navCompanyName + "Membership Card$5ecfc871-5d82-43f1-9c54-59685e82318d] c on c.[Contact No_]=mc.[Contact No_] AND mt.[Visible Type]=0 AND mt.[Lookup Type]=0" + 
                                          sqlWhere;
        ...
        public GiftCard GetGiftCartBalance(string cardId, string type)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "SELECT mt.[Expiring Date] AS Exp,mt.[Amount],li.[Trans_ Currency] AS Cur1," +
                                          "(SELECT SUM([Amount]) FROM [" + navCompanyName + "Voucher Entries$5ecfc871-5d82-43f1-9c54-59685e82318d] " +
                                          ...
                                          "LEFT JOIN [" + navCompanyName + "Transaction Header$5ecfc871-5d82-43f1-9c54-59685e82318d] li ON li.[Receipt No_]=mt.[Created by Receipt No_] " +
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        ...
                        {
                            card.EntryType = type;
                            if (reader["Amt"] == null || reader["Amt"] == DBNull.Value)
                                card.Balance = SQLHelper.GetDecimal(reader, "Amount");
                            else
